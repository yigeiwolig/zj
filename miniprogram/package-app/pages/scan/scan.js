...
Page({
  ...
  async checkOtaConnection(deviceId) {
    try {
      ...
      // F2 LONG / F3 MAX：工程机与量产固件均跳过 OTA 门禁（避免连上即断、完全无状态）
      ...
      if (isF3Max) {
        console.log('🔍 [checkOtaConnection] 当前为 F3 MAX，跳过 OTA 校验');
        return true;
      }
