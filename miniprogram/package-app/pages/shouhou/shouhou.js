...
function buildShippingDisplay(method, fee) {
...
*/
          ...
          success: (res) => {
            if (res && res.confirm) {
              ...
              wx.navigateTo({
                ...
                fail: (err2) => {
                  console.error('[checkDeviceBeforeRepair] navigateTo 失败:', err2);
  ...
  loadParts(modelName) {
    ...
    // 从 shouhou 集合读取，与本地 DB_PARTS 合并（有默认列表时不再「只显示云端条数」）
    db.collection('shouhou').where({ modelName: modelName }).get().then(res => {
      ...
      const listForPaint = parts.map(p => ({ ...p, preselected: false }));
      this.setData({ currentPartsList: listForPaint, selectedCount, totalPrice });
      if (parts.some(p => p.preselected)) {
        const that = this;
        setTimeout(function () {
          that.setData({ currentPartsList: parts, showPreselectTip: true });
  ...
  reCalcFinalPrice(cart = this.data.cart) {
    ...
    this.setData({
      ...
      shippingFee: fee,
      finalTotalPrice: goodsTotal + fee,
      ...buildShippingDisplay(shippingMethod, fee)
    });
  },
