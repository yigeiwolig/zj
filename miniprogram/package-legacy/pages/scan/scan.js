...
/** 自动校准入口：F2(非PRO) + F1 Ultra + F3 MAX */
...
function buildRemoteAssistAdminExtras(model, settingState, bleLinked) {
  ...
  if (isF3MaxModel(model)) {
    patch.f3HeightMonitorVisible = linked;
  }
...
Page({
  ...
  /** 进入控制中心：弹「按价格找型号」引导（普通用户仅首次；管理员每次都弹，便于查看效果） */
  ...
  onLoad(options) {
    ...
    // 由新版页面转来的旧机远协：会话结束后自动退回新版页面
    this._raLegacyEntry = !!(options && options.raLegacy);
    // 🔴 计算导航栏高度（适配所有机型）
    this.calcNavBarInfo();
  ...
  /** 陀螺仪版设备不能用这套旧界面控制：把会话交回新版控制台 */
  _handoffRemoteAssistToNewScan(session) {
    ...
    try {
      wx.setStorageSync(REMOTE_ASSIST_STORAGE_KEY, {
        ...
        remoteAssistSessionProductKey: session.productKey || '',
        remoteAssistUserAccepted: !!session.userAccepted
      });
    } catch (e) { /* ignore */ }
    this._stopRemoteAssistSessionPollers();
    this._showCustomToast('新版设备，返回新版控制台', 'none', 2000);
    wx.navigateBack({
      delta: 1,
      fail: () => {
        wx.redirectTo({ url: '/package-extra/pages/scan/scan', fail: () => {} });
      }
    });
    return true;
  },
  ...
  async _acceptRemoteAssistSession(sessionId) {
    ...
    try {
      ...
      const st = (r.session && r.session.deviceState) || {};
      if (st.f3DeviceVariant && st.f3DeviceVariant !== 'tof') {
        if (this._handoffRemoteAssistToNewScan(r.session)) return;
      }
  ...
  _clearRemoteAssistLocal(reason) {
    ...
    this.setData({
      ...
      if (wasAdmin && this._raLegacyEntry) {
        this._raLegacyEntry = false;
  ...
  handleAutoCalibrate() {
    ...
    // F2 除 PRO 外 + F1 Ultra + F3 MAX；F1 MAX 等无此能力
