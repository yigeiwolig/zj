...
Page({
  ...
  handleIntercept(type) {
    ...
    console.log('[home] 🔴 截屏/录屏检测，立即跳转到封禁页');
    ...
    this._getLocationAndDeviceInfo().then(locationData => {
      wx.cloud.callFunction({
        ...
        success: (res) => {
          console.log('[home] banUserByScreenshot 调用成功，类型:', type, '结果:', res);
        },
        fail: (err) => {
          console.error('[home] banUserByScreenshot 调用失败:', err);
        }
      });
    }).catch(() => {
      ...
      wx.cloud.callFunction({
        ...
        success: (res) => {
          console.log('[home] banUserByScreenshot 调用成功（无位置信息）');
        },
        fail: (err) => {
          console.error('[home] banUserByScreenshot 调用失败:', err);
        }
