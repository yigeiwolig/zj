  ...
  loadPendingRepairs() {
    ...
    db.collection('shouhou_repair')
      ...
      .then(res => {
        ...
        // 统一兜底质保字段：优先用工单顶层，其次用工单里的 device 字段重新计算
        const now = new Date();
        const normalized = filtered.map(item => {
          ...
          // 展示型号优先使用维修单内设备型号，避免顶层 model 与用户实际报修设备不一致
          const displayModel = (item.device && item.device.productModel) || item.model || '未知型号';

          // remainingDays 缺失时，尝试复用 device.days（部分历史工单仅在 device 里有）
          ...
          setTimeout(() => {
            ...
            const tutorialUrl = (() => {
              if (isShouhou) {
                ...
                q.push('autoUnlock=1');
                return '/pages/shouhou/shouhou' + (q.length ? ('?' + q.join('&')) : '');
              }
              const q = [];
              if (modelName) q.push('model=' + encodeURIComponent(modelName));
              q.push('justConfirmed=1');
              return '/pages/azjc/azjc' + (q.length ? ('?' + q.join('&')) : '');
            })();
            wx.navigateTo({
              url: tutorialUrl,
              animationType: 'none',
...
备注（到付直接拒收，无需派送）`;
  ...
  onFillRepairRequireUserPayChange(e) {
    ...
    this.setData(patch, () => {
      if (v) {
        this.showAutoToast('提示', '已开启用户支付维修费，用户需在「我的」卡片中完成支付');
      }
    });
  },
  ...
  submitFillRepair() {
    ...
    if (fillRepairRequireUserPay) {
      ...
      if (validItems.length === 0) {
        setTimeout(() => {
          this.showAutoToast('提示', '请至少添加一项有名称且单价大于 0 的维修项目');
        }, 100);
        return;
      }
      ...
      if (totalPrice <= 0) {
        setTimeout(() => {
          this.showAutoToast('提示', '需要用户付款时合计须大于 0');
        }, 100);
        return;
      }
      if (fillRepairTrackingId.trim()) {
        setTimeout(() => {
          this.showAutoToast('提示', '用户付款完成前请勿填写寄回运单号');
        }, 100);
        return;
      }
      ...
      db.collection('shouhou_repair').doc(currentFillRepairItem._id).update({
        ...
        this.loadMyOrders();
        this.showAutoToast('提交成功', '用户可在「我的」需寄回卡片中点击「立即支付」');
    ...
    if (namedItems.length === 0) {
      setTimeout(() => {
        this.showAutoToast('提示', '请至少添加一个维修项目');
      }, 100);
      return;
    }
    ...
    db.collection('shouhou_repair').doc(currentFillRepairItem._id).update({
      ...
      this.loadMyOrders();
      this.showAutoToast('提交成功', '售后单已提交，已寄出快递\n用户端已更新状态');
    }).catch(err => {
      this.hideMyLoading();
      console.error('提交失败:', err);
      setTimeout(() => {
  ...
  doRepairPayment(repairId, repairItems, totalPrice) {
    ...
    const isAdmin =
      ...
      (app && app.globalData && (app.globalData.isAdmin || app.globalData.isAuthorized));
  ...
  async startConnect() {
    ...
    // 🔴 管理员测试模式：跳过真实蓝牙连接，直接模拟一个设备
    // 只要系统识别为管理员/已授权管理员（不用手动切换管理模式），就直接走这里
    const app = getApp();
    const isAdminLike = this.data.isAdmin 
      || this.data.isAuthorized 
      || (app && app.globalData && (app.globalData.isAdmin || app.globalData.isAuthorized));

    if (isAdminLike) {
      console.log('[startConnect] 管理员测试模式：自动识别为管理员，跳过真实蓝牙连接，直接模拟设备');
      // 随机生成一个假的 NB 设备名，确保能通过 handleDeviceBound 里的 NB 校验
      const mockSn = String(Math.floor(100000 + Math.random() * 900000)); // 6 位数字
      const mockName = 'NB' + mockSn;

      // 在界面上展示“已连接”，方便你确认状态
      this.setData({
        isScanning: false,
        bluetoothReady: true,
        connectStatusText: '管理员测试模式：设备已连接（模拟）',
        connectedDeviceName: mockName
      });

      // 走和真实蓝牙一样的后续逻辑（提取 SN、调用 bindDevice 等）
      const mockDevice = {
        deviceId: 'admin-mock-device-' + mockSn,
        name: mockName,
        localName: mockName
      };
    ...
    // 🔴 检查 app_config.is_active，如果为 false 则全局审核模式，模拟蓝牙连接成功
    const db = wx.cloud.database();
    try {
  ...
  handleDeviceBound(device) {
    ...
    this.setData({ 
      ...
      isDeviceLocked: true // 🔴 修复：先锁定，等待验证通过后再解锁显示表格
    });
  ...
  _bindDeviceWithRetry(sn, displayName, retryCount = 0) {
    ...
    wx.cloud.callFunction({
      ...
      success: res => {
        ...
        // 🔴 修复：bluetoothReady 已经在 handleDeviceBound 中设置了
        // 这里只需要更新连接状态文本
        this.setData({
        ...
        if (result && result.success) {
          ...
          // 🔴 修复：其他成功情况（status 不是上述两种情况）
            ...
            console.log('[bindDevice] 其他成功情况，直接解锁，status:', result.status);
            // 其他成功情况，直接解锁显示表格
            this.setData({ 
              isDeviceLocked: false,
              currentSn: sn
      ...
      fail: (err) => {
        ...
        this.setData({
          bluetoothReady: true,
          connectStatusText: '连接成功，但验证失败',
          isDeviceLocked: false, // 🔴 修复：验证失败也允许填写表单
          currentSn: sn
        });
