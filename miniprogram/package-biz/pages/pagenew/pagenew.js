...
Page({
  ...
  _isDevtoolsLocalPreviewUrl(url) {
    const s = String(url || '').toLowerCase();
    if (!s) return false;
    if (s.indexOf('http://tmp') === 0 || s.indexOf('https://tmp') === 0) return true;
    if (s.indexOf('http://usr') === 0 || s.indexOf('https://usr') === 0) return true;
    if (/^https?:\/\/(127\.0\.0\.1|localhost)(:\d+)?\//i.test(s)) return true;
    if (s.indexOf('wxfile://') === 0) return true;
    return false;
  },

  // 图片失败时二次检测重试一次（cloud 临时链接 / COS https 刷新参数）
  async onPagenewCoverError(e) {
    ...
    if (this._isDevtoolsLocalPreviewUrl(cover)) return;
    ...
    try {
      ...
      } else if (cover.startsWith('https://')) {
  ...
  handleUploadCover: function() {
    ...
    var idx = this.data.currentIndex;
    var item = this.data.products[idx];
    shopImagePrepare.chooseAndPrepare('pagenew').then(function(path) {
        ...
        _this.uploadCoverToCos(path).then(id => {
          return new Promise(function(resolve, reject) {
            ...
            _this.setData({ [`products[${idx}].cover`]: id });
            wx.showToast({ title: '封面已更新', icon: 'success' });
          });
        }).catch(err => {
          console.error('[pagenew] 封面上传或保存失败:', err);
          const msg = String((err && err.message) || (err && err.errMsg) || '上传失败');
          wx.showToast({
            title: msg.length > 28 ? '上传失败，请检查 COS 域名或云函数' : msg,
            icon: 'none',
            duration: 3200
          });
        }).finally(function() {
        ...
        console.error('[pagenew] 选择封面失败:', err);
      });
  },
  ...
  handleCardClick: function(e) {
    ...
    wx.navigateTo({
      url: `/package-app/pages/shop/shop?jumpNumber=${product.jumpNumber}`,
  ...
  dismissTransientModals: function() {
    if (this.data.showCustomEditModal) {
      this.closeCustomEditModal();
    }
  },
