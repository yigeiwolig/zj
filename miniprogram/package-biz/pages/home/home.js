...
Page({
  data: {
    ...
    shops: [], // 轮盘展示列表（普通用户：50km 内已授权；管理员模式：全量）
    shopsFull: [], // 全量店铺（含距离等预处理结果），用于筛选与管理员编辑
  ...
  // 更新所有店铺的营业状态
  updateShopStatus() {
    const { shopsFull, activeItem, isDetailOpen } = this.data;
    if (!shopsFull.length) {
      this.updateWheel();
      return;
    }
    const updatedFull = shopsFull.map(item => {
      const serviceSet = {};
    ...
    this.setData({ shopsFull: updatedFull });
    const viewShops = this.applyNearbyStorePolicy();

    if (isDetailOpen && activeItem && (activeItem.id || activeItem._id)) {
      const currentItem = viewShops.find(item =>
      ...
      if (isRetry && this._isLoadingShops) {
        ...
        const checkLoading = () => {
          if (!this._isLoadingShops || waitCount > 100) {
            resolve(this.data.shopsFull || this.data.shops || []);
          } else {
            waitCount++;
          ...
          try {
            ...
            let shops = res.data
              ...
              .map(item => ({
                ...
                // 未传或 true 视为已授权；仅当显式为 false 时在附近门店中对用户隐藏
                isStoreAuthorized: item.isStoreAuthorized !== false
              }));
            ...
            if (isTestMode) {
              ...
              if (isRetry) {
                ...
                this.setData({ shops, shopsFull: shops });
                // 处理数据（计算距离、营业状态、排序）
                this.preprocessData();
                this.updateWheel();
              ...
              if (oldTestShops.length !== 6) {
                ...
                try {
                  ...
                  this.setData({ shops: testShopsOnly, shopsFull: testShopsOnly });
                  this.preprocessData();
                  this.updateWheel();
  ...
  preprocessData() {
    const { shops, shopsFull, userLocation } = this.data;
    if (!userLocation) {
      console.warn('用户位置未获取，无法计算距离');
      return;
    }

    const source = (shopsFull && shopsFull.length > 0) ? shopsFull : shops;
    ...
    this.setData({ shopsFull: list });
    this.applyNearbyStorePolicy();
    return list;
  },

  /** 无可用门店时：第 1 张说明无门店，第 2 张单独招商（可滑动切换） */
  buildNearbyEmptyPlaceholders() {
    const base = {
      sub: '',
      status: '营业中',
      statusColor: '#00C853',
    ...
    return [
      {
        ...base,
        id: '__nearby_empty_0',
        _id: '__nearby_empty_0',
        name: '附近暂无门店',
        emptyStoreLead: `当前 ${NEARBY_STORE_RADIUS_KM} 公里内暂无已授权门店。`,
        emptyStoreCalloutLine: '若在其它区域需要门店',
        emptyStoreCalloutAction: '请联系管理员协助查询。',
        isNearbyEmptyHint: true,
        isNearbyInvestCard: false,
        isOpen: false,
        dist: String(NEARBY_STORE_RADIUS_KM)
      },
      {
        ...base,
        id: '__nearby_invest_0',
        _id: '__nearby_invest_0',
        name: '招商合作',
        investHintRows: ['欢迎线下门店入驻', '与品牌合作洽谈。'],
        isNearbyEmptyHint: false,
        isNearbyInvestCard: true,
        isOpen: true,
        dist: String(NEARBY_STORE_RADIUS_KM)
      }
    ];
  ...
  /** 普通用户仅看 50km 内已授权门店；管理员模式看全量。返回当前轮盘使用的列表。 */
  applyNearbyStorePolicy() {
    const full = this.data.shopsFull || [];
    const isAdminMode = this.data.isAdmin === true;
    ...
    if (!full.length) {
      ...
      const placeholders = this.buildNearbyEmptyPlaceholders();
      this.setData({ shops: placeholders });
      return placeholders;
    }

    if (isAdminMode) {
      this.setData({ shops: full });
      return full;
    }

    const filtered = full.filter(item => {
      if (item.isStoreAuthorized === false) return false;
      if (!item.latitude || !item.longitude) return false;
      const km = parseFloat(item.dist);
      if (Number.isNaN(km)) return false;
      return km <= NEARBY_STORE_RADIUS_KM;
    });
  ...
  toggleAdminMode() {
    ...
    this.setData({ isAdmin: nextState, scroll: 0, target: 0 }, () => {
      this.applyNearbyStorePolicy();
      this.updateWheel();
  ...
  onBgTap(e) {
    ...
    if (this.data.isAuthorized && !this.data.isAdmin) {
      this.setData({ isAdmin: true, scroll: 0, target: 0 }, () => {
        this.applyNearbyStorePolicy();
        this.updateWheel();
  ...
  onAddShop() {
    ...
    const base = (this.data.shopsFull && this.data.shopsFull.length)
      ? this.data.shopsFull
      : this.data.shops;
    const list = [...base];
  ...
  onCardTap(e) {
    ...
    if (item && item.isNearbyEmptyHint) {
      if (this.data.currentIndex !== index) {
        this.setData({ target: index });
        this.loop();
        return;
      }
      this.showAutoToast('提示', '滑动可切换到「招商合作」');
      return;
    }
    if (item && item.isNearbyInvestCard) {
      if (this.data.currentIndex !== index) {
        this.setData({ target: index });
        this.loop();
        return;
      }
      this.onNearbyInvestTap();
      return;
    }
    if (this.data.currentIndex !== index) {
      this.setData({ target: index });
  ...
  onNearbyInvestTap() {
    wx.navigateTo({
      ...
      fail: (err) => {
        console.error('[home] 跳转招商/联系页失败', err);
        this.showAutoToast('提示', '暂时无法打开，请稍后再试');
      }
    });
  },
  ...
  _doChooseImage(sourceType) {
    ...
    shopImagePrepare.chooseAndPrepare('home', { sourceType }).then((tempFilePath) => {
            ...
            try {
              ...
              if (that.data.isEditing) {
                ...
                if (shopId && !shopId.toString().startsWith('new_')) {
                  ...
                  const shopsFull = that.data.shopsFull.map(shop => {
                    if ((shop._id && shop._id === shopId) || (shop.id === shopId)) {
                      shop.img = cloudFileID;
                    }
                    return shop;
                  });
                  that.setData({ shopsFull });
                  that.applyNearbyStorePolicy();
                  that.updateWheel();
  ...
  // --- 2. 打开详情 (时间解析逻辑) ---
  openDetail(index) {
    const pre = this.data.shops[index];
    if (pre && (pre.isNearbyEmptyHint || pre.isNearbyInvestCard)) return;
    ...
    query.select(`#card-${index}`).boundingClientRect(rect => {
      if (!rect) return;
      const item = this.data.shops[index];
      if (item && (item.isNearbyEmptyHint || item.isNearbyInvestCard)) return;
  ...
  cancelAddShop() {
    const list = (this.data.shopsFull && this.data.shopsFull.length)
      ? this.data.shopsFull
      : this.data.shops;
    // 删除临时的新卡片
  ...
  async saveShopToCloud(shopData, isNew = false) {
    return new Promise((resolve, reject) => {
      const dataToSave = {
        ...
        isStoreAuthorized: shopData.isStoreAuthorized !== false,
        updateTime: new Date()
      };
  ...
  async saveEdit() {
    const { editData, shopsFull, shops } = this.data;
    const lookupList = (shopsFull && shopsFull.length) ? shopsFull : shops;
    ...
    if (isNewShop) {
      // 新店铺：从全量列表中找到临时创建的店铺
      foundIndex = lookupList.findIndex(s => s.id === editData.id);
      if (foundIndex === -1) {
        this.showAutoToast('提示', '店铺不存在');
        return;
      }
      item = lookupList[foundIndex];
    } else {
      // 编辑已有店铺：优先通过 _id 查找，其次通过 id 查找
      foundIndex = lookupList.findIndex(s => {
  ...
  deleteShop() {
    this.showMyDialog({
      ...
      success: async (res) => {
        if(res.confirm) {
          const { activeItem, shopsFull, shops } = this.data;
          const baseList = (shopsFull && shopsFull.length) ? shopsFull : shops;
          const shopId = activeItem._id || activeItem.id;
          
          // 如果是新店铺（还未保存到云数据库），直接删除本地数据
          if (shopId && shopId.toString().startsWith('new_')) {
            const list = baseList.filter(item => item.id !== shopId);
            this.closeDetail();
            setTimeout(() => {
              this.setData({ shopsFull: list, scroll: 0, target: 0 });
              this.preprocessData();
              this.updateWheel();
            }, 500);
            return;
